...
//!
//! VENDORED, PATCHED, AND TEMPORARY — not fig code. A verbatim copy of Zig
//! 0.16.0's own `lib/zig/compiler/test_runner.zig` (MIT, © Zig contributors)
//! carrying exactly ONE edit, marked `fig patch` below. Used only by `zig build
//! fuzz`; `zig build test` and `zig build check` keep using the stock runner
//! that ships with the toolchain, so this file cannot affect the release gate.
//!
//! Why it has to exist: `zig build test --fuzz` does not compile on Zig 0.16.0.
//! The runner's fuzz path calls `std.debug.writeStackTrace` with the value of
//! `@errorReturnTrace()`, but those types are not the same one —
//!
//!     @errorReturnTrace() -> ?*std.builtin.StackTrace
//!     writeStackTrace(st: *const std.debug.StackTrace, ...)     <- wrong fn
//!     writeErrorReturnTrace(et: *const std.builtin.StackTrace, ...)  <- right fn
//!
//! — so the build dies with "expected type '*const debug.StackTrace', found
//! '*builtin.StackTrace'". It is upstream's bug, not ours, and it hits every
//! 0.16.0 project that tries to fuzz. It survived upstream CI because the fuzz
//! path is only ever semantically analyzed under `-ffuzz`, which is also why a
//! plain `zig build test` is unaffected. The runner's three other error-trace
//! sites (~lines 146, 314, 440) already call `dumpErrorReturnTrace` correctly;
//! the fuzz one is simply the odd one out.
//!
//! HOW TO DELETE THIS FILE (please do, at the first opportunity): when a Zig
//! version whose stock runner already calls `writeErrorReturnTrace` here lands,
//! drop this file and delete the `.test_runner` field from the fuzz artifact in
//! build.zig. Nothing else references it. To check, diff against the toolchain
//! in use:
//!
//!     diff tools/fuzz_test_runner.zig "$(zig env | grep lib_dir | cut -d'"' -f4)/compiler/test_runner.zig"
//!
//! A one-line diff at the `fig patch` marker means the bug is still there and
//! this file is still earning its keep. No diff means upstream fixed it — delete
//! this. A LARGE diff means the runner has moved on underneath us and this copy
//! is stale: re-vendor from the current toolchain and re-apply the patch (or, if
//! the bug is gone, just delete). Nothing detects that staleness automatically,
//! which is precisely why this is scoped to `fuzz` alone.
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    p: {
                        if (@errorReturnTrace()) |trace| {
                            // fig patch — THE ONLY EDIT IN THIS FILE. Upstream
                            // 0.16.0 has `writeStackTrace` here, which does not
                            // compile: `@errorReturnTrace()` hands back a
                            // `*builtin.StackTrace`, not a `*debug.StackTrace`.
                            // See this file's header before touching or removing.
