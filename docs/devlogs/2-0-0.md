...
The star of this release is the new configuration format, eponymously named Fig.
...
Fig is not, however, a literal superset of JSON—`fig get file.json -i fig` doesn't work. But there is no reason to need that, since `fig` supports JSON anyway.


Fig is the answer to a question

The central ideas of the design are:
- **Literal-else-string.** There are very few literals, so almost all strings exist as-is, without quotes or escaping.
- **Repeated-marker depth.** In Fig, the `>` character indicates nesting depth.
- **Accept many.** Fig is designed to be dead simple and easy to author and modify (including over SSH at 3AM with no tooling). Fig is also a superset of JSON, so you can copy/paste JSON into a `.fig` file and it just works.
- **Emit (a beautiful) one.** Use `fig get myfile -o fig` to try it out. Your file will look like a beautiful, hand-authored configuration file straight out of the tool.
- **Preserve intent.** Fig supports explicit typing via `key: type = value` syntax. So `val: string = [ 1 + 2 ]` works, as does `leading_zero: int = 09` and `trailing_period: int = 1.`. Fig, of course, also preserves comments across formats.
