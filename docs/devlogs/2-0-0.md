...
# (the printer's mirror of the flattener above):
...
I plan to use Fig in my personal journal, and also provide support for it in my Diaryx project.
...
- The Fig format! (I have chosen to stylize `fig`, the library, as inline code/raw text, because it is intended for use by programmers only. Fig, the format, is stylized as capitalized normal text—not like TOML, YAML, or JSON, because it is not an acronym or initialism, but simply an abbreviation of "configure." But it is a proper noun.)
...
- The native `fig` authoring dialect is now a first-class `Format`/`FigFormat` member in the C ABI, Rust, and TypeScript bindings — parse, edit, and serialize it through the same API as every other format (previously only reachable via the CLI). `EmbedType::FrontmatterFig` (```` ```fig ```` fenced frontmatter) is now editable through `fig_embed_*`/`Embed`, not just extractable.