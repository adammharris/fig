...
pub fn Editor(comptime Language: type) type {
    ...
    return struct {
        ...
        // the table's disjoint regions (see `gatherTableRegions`) and rebuild the
        ...
        fn spliceOutRegions(self: *Self, regions: []const Region) !void {
            const source = self.source.items;
            var out: std.ArrayList(u8) = .empty;
            defer out.deinit(self.allocator);
            var pos: usize = 0;
        ...
        /// Lets `gatherElementRegions` locate an empty element's header.
...
fn headerLineRegion(source: []const u8, seg_span: Span, style: CommentStyle) ?Region {
...
fn entryLineRegion(source: []const u8, kv_span: Span, style: CommentStyle) Region {
    return .{
...
fn gatherTableRegions(parsed: Document, source: []const u8, allocator: std.mem.Allocator, node: AST.Node, include_header: bool, out: *std.ArrayList(Region)) std.mem.Allocator.Error!void {
    if (include_header) {
...
fn gatherAotRegions(parsed: Document, source: []const u8, allocator: std.mem.Allocator, node: AST.Node, out: *std.ArrayList(Region)) std.mem.Allocator.Error!void {
...
fn gatherElementRegions(parsed: Document, source: []const u8, allocator: std.mem.Allocator, elem: AST.Node, search_from: usize, out: *std.ArrayList(Region)) std.mem.Allocator.Error!void {
...
fn headerSegmentSpan(source: []const u8, region: Region, depth: usize) ?Span {
